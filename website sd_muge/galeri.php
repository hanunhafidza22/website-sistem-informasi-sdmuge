...
<body class="host_version">
  ...
  <div class="all-title-box" style="background-image: url('admin/asset/image/<?= $sekolah['gambar']?>')" >
    <div class="container text-center" >
      <h1>GALERI<span class="m_1">SD Muhammadiyah Bojonggede</span></h1>
    </div>
  </div>
  ...
  <div id="overviews" class="section wb">
    <div class="container">
      <?php
        ...
        $galeri = mysqli_query($koneksi, "SELECT * FROM tbl_galeri ORDER BY id ASC");
        $totalImages = mysqli_num_rows($galeri);

      if ($totalImages > 0) {
      $index = 0;
      while ($dataGaleri = mysqli_fetch_array($galeri)) {
        if ($index >= $offset && $index < ($offset + $limit)) {
        ?>
        <main class="container">
          <div class="col-3">
            <a href="admin/asset/image/galeri/<?= $dataGaleri['foto'] ?>" target="_blank" class="thumbnail-link" >
            <div class="thumbnail-box">
              <div class="thumbnail-img" style="background-image: url('admin/asset/image/galeri/<?= $dataGaleri['foto'] ?>')">
              </div>
              <div class="thumbnail-text">
                <?= $dataGaleri['keterangan'] ?>
              </div>
            </div>
            </a>
          </div>
